...
SELECT a.createddate::date                        AS reporting_date,
                             CASE
                                 WHEN o.original_opportunity_owner_role__c ilike '%power%'
                                     THEN 'Power' -- uses booked for role
                             ...
                             -- #TODO: REVIEW - should we change this to use "appointment_booked_by_role__c" on the Appointment object?
...
--                              0                                              AS correct_contact_conversation_calls,
                             ...
                             -- #TODO: REVIEW - Needs to be updated to use a.original_appointment_booked_for_role__c. Does not exist yet
                      ...
                      FROM {{ ref('raw_appointment') }} a
                               ...
                               LEFT JOIN {{ ref('raw_user') }} uf ON a.booked_for__c = uf.id -- #TODO: REVIEW - should we use the Appointment fields instead?
                           WHERE 1 = 1
                        ...
                        AND a.sdr_product_credit__c = 'true'
                        AND (a.createddate >= '2024-01-01' AND o.saasoptic_grouping_id__c IS NULL)
                      ...
                      GROUP BY reporting_date,
                               ...
                               u.name, o.contactid,
                               uf.name, uf.isactive,
                               a.booked_for_role__c,
                               a.original_booked_by_role__c,
                               acc.brand_name_formula__c, o.hcm_legacy_id__c,
                               opportunity_main_product, a.ssr__c